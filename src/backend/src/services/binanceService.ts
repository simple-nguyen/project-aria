...
import {
  ...
  sortDepthAsc,
  sortDepthDesc,
  reduceDepthWithTotal,
} from '../utils/helper';

const BINANCE_WS_URL = 'wss://stream.binance.com:9443/stream';

interface BinanceServiceEvents {
  trade: (trade: MarketTrade) => void;
  depth: (depth: OrderBook) => void;
  ticker: (ticker: MarketTicker) => void;
  error: (error: AppError) => void;
}

interface BinanceServiceInterface {
  on<U extends keyof BinanceServiceEvents>(event: U, listener: BinanceServiceEvents[U]): this;
  emit<U extends keyof BinanceServiceEvents>(
    event: U,
    ...args: Parameters<BinanceServiceEvents[U]>
  ): boolean;
}

export class BinanceService extends EventEmitter implements BinanceServiceInterface {
  ...
  public connect(): void {
    try {
      ...
      this.ws.on('error', (error: Error) => {
        ...
        this.emit(
          'error',
          new AppError('WebSocket connection error', ErrorCodes.WEBSOCKET_CONNECTION_FAILED, 500)
        );
      });
  ...
  private handleMessage(data: WebSocket.Data): void {
    ...
    try {
      ...
      if (error instanceof SyntaxError) {
        ...
        this.emit(
          'error',
          new AppError('Invalid market data received', ErrorCodes.INVALID_MARKET_DATA, 500)
        );
      } else {
  ...
  private handleDepthMessage(message: any): void {
    ...
    const orderBook: OrderBook = {
      ...
      bids: message.bids.map(parseDepthFloats).sort(sortDepthDesc).reduce(reduceDepthWithTotal, []),
      asks: message.asks.map(parseDepthFloats).sort(sortDepthAsc).reduce(reduceDepthWithTotal, []),
    };
  ...
  private resubscribe(): void {
    for (const symbol of this.activeSymbols) {
      this.subscribe(symbol, streams).catch(error => {
